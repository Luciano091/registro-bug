        ...
        <section className="mt-7 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {view === 'estabelecimentos' ? (
            ...
            <div className="divide-y divide-slate-100">
              ...
              {leads.map((lead) => <article key={lead.id} className="grid gap-4 p-4 sm:grid-cols-[1fr_auto] sm:items-start sm:p-5"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">{lead.estabelecimento}</h2><span className="rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-bold uppercase text-violet-700">{lead.status}</span></div><p className="mt-1 text-sm text-slate-600">{lead.nome} · {lead.telefone}{lead.cidade ? ` · ${lead.cidade}` : ''}</p><p className="mt-1 text-xs text-slate-400">{lead.email}</p>{lead.mensagem && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm italic text-slate-600">"{lead.mensagem}"</div>}</div><select value={lead.status} onChange={(e) => updateLead(lead, e.target.value)} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600"><option value="novo">Novo</option><option value="contatado">Contatado</option><option value="convertido">Convertido</option><option value="descartado">Descartado</option></select></article>)}
            </div>
          )}
        </section>
      </main>
