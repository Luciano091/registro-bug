...
const emptyForm = { nome: '', slug: '', email: '', telefone: '', documento: '', plano: 'Essencial', status: 'trial', trial_ate: '' };
...
export default function PlatformDashboard() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('platformToken')) {
      navigate('/ritmesa-admin/login', { replace: true });
      return;
    }
    ...
    <div className="min-h-screen bg-[#f5f7fa] text-slate-900">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <button onClick={() => { localStorage.removeItem('platformToken'); navigate('/ritmesa-admin/login'); }} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900"><LogOut size={17} /> <span className="hidden sm:inline">Sair</span></button>
        </div>
      </header>
